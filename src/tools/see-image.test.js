...
Deno.test("see_image invokes fallback model with local image and default prompt", async () => {
    const cwd = await Deno.makeTempDir({ prefix: "harns-see-image-" });
    try {
...
Deno.test("see_image resolves attachment refs from the session image directory", async () => {
    ...
    const tempHome = await Deno.makeTempDir({ prefix: "harns-see-image-home-" });
    const cwd = await Deno.makeTempDir({ prefix: "harns-see-image-attachment-" });
    try {
